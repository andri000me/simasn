...
class Pegawai_model extends CI_Model{
    ...
    function costum()
    {
        ...
        if(!empty($link['ukid'])):
            $this->db->where(array('skpd_id' => decrypt_url($link['ukid'])));
        endif;
        if(!empty($link['golid'])):
    ...
    {
        $this->db->where('mus.nip', $pegawai['nip']);
    ...
    function updateStatuspns($pegawai, $post)
    {
        ...
        if(isset($_FILES['sk_pnsnonaktif']['name'])){
            ...
            $config['file_name'] = 'sk_pnsnonaktif_'.$pegawai;
        ...
        $data_update['ket_statuspns'] = $post['statuspnsket'];
