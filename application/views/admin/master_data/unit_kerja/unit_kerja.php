...
<body class="fixed-header horizontal-menu horizontal-app-menu ">
    ...
    <div class="page-container ">
        ...
        <div class="page-content-wrapper ">
            ...
            <div class="content ">
                ...
                <div class="col-md-12">
                    <label>
                        <h4><b>MASTER UNIT KERJA</b></h4>
                    </label>
                    <div class="row">
                        <div class="col-md-6">
                            <?= form_open('admin/master_data/search_unit_kerja', 'id="formCari"');?>
                            <div class=" form-group">
                                <input type="text" name="kata_cari" class="form-control" placeholder="-Unit Kerja-">
                            </div>
                            <?= form_close(); ?>
                        </div>
                        <div class="col-md-6">
                            <div class="btn-group">
                                ...
                                <a class="btn btn-info btn-sm" type="button" href="<?= base_url().'admin/master_data/unit_kerja/unduh_excel'; ?>">
                                    <i class="fa fa-download"></i>&nbsp;
                                    <span class="bold">Unduh</span>
                                </a>
                                <a class="btn btn-warning btn-sm" type="button" href="<?= base_url().'admin/master_data/unit_kerja/cetak_pdf'; ?>">
                                    <i class="fa fa-print"></i>&nbsp;
                                    <span class="bold">Cetak</span>
                                </a>
                                <a class="btn btn-success btn-sm" type="button" href="<?= base_url().'admin/master_data/unit_kerja/form_tambah_unit_kerja'; ?>">
                    ...
                    <div style="overflow-x: auto;" class="col-md-12 container container-fixed-lg p-0">
                        ...
                        <table class="table table-sm table-hover table-striped table-bordered m-t-10">
                            ...
                            <tbody>
                                <?php $no=1; foreach($unit_kerja as $b): ?>
                                <tr>
                                    <td class="text-nowrap p-2 text-center"><?= $uri4 + $no++ ?></td>
                                    <td class="text-nowrap p-2 text-center">
                                      <a type="button" href="<?= base_url().'admin/master_data/unit_kerja/form_ubah_unit_kerja/'.encrypt_url($b['id_unitkerja']); ?>" class="text-white btn btn-xs btn-success" title="Edit Data"><i class="fa fa-pencil"></i></a>
                                      <a type="button" href="#" data-target="#hapusbid<?= encrypt_url($b['id_unitkerja']) ?>" data-toggle="modal" class="text-white btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
                                    </td>
                                    <td class="text-nowrap p-2"><?= lindungi($b['nm_unitkerja']) ?></td>
                                    <td class="text-nowrap p-2 text-center">
                                        <input type="checkbox" data-init-plugin="switchery" name="status_unitkerja" data-size="small" data-color="primary" value="<?= encrypt_url($b['id_unitkerja']) ?>" <?= $b['status_unitkerja']==1 ? 'title="Aktif" checked':'title="Tidak Aktif"'; ?> />
                                    </td>
                                </tr>
                                <!-- MODAL AKTIFKAN DATA -->
                                <div class="modal fade slide-up" id="hapusbid<?= encrypt_url($b['id_unitkerja']) ?>" tabindex="-1" role="dialog" aria-hidden="true">
                                        ...
                                        <div class="modal-content-wrapper">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    <a type="button" href="<?= base_url().'admin/master_data/aksi_unit_kerja/hapus_unit_kerja/'.encrypt_url($b['id_unitkerja']); ?>" class="btn btn-danger btn-cons pull-left inline">Yes</a>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('input[name=status_unitkerja]').change(function(){
                var mode= $(this).prop('checked');
                var id=$( this ).val();
                $.ajax({
                    url : "<?php echo base_url();?>admin/get_data/update_status_unitkerja",
                    method : "POST",
                    data : {mode:mode, id:id},
                    dataType : 'json',
                    success: function(data){
                        for(i=0; i<data.length; i++){
                            if(data[i].status_unitkerja==1){
                                $('input[name=status_unitkerja]').attr('checked', 'checked').attr('title', 'Aktif');
                            }else{
                                $('input[name=status_unitkerja]').removeAttr('checked', 'checked').attr('title', 'Tidak Aktif');
                            }
                        }
