...
<body class="fixed-header horizontal-menu horizontal-app-menu ">
    ...
    <div class="page-container ">
        ...
        <div class="page-content-wrapper ">
            ...
            <div class="content ">
                ...
                <div class="col-md-12">
                    <label>
                        <h4><b>MASTER BIDANG</b></h4>
                    </label>
                    <div class="row">
                        <div class="col-md-6">
                            <?= form_open('admin/master_data/search_bidang', 'id="formCari"');?>
                        ...
                        <div class="col-md-6">
                            <div class="btn-group">
                                ...
                                <a class="btn btn-info btn-sm" type="button" href="<?= base_url().'admin/master_data/bidang/unduh_excel'; ?>">
                                    <i class="fa fa-download"></i>&nbsp;
                                    <span class="bold">Unduh</span>
                                </a>
                                <a class="btn btn-warning btn-sm" type="button" href="<?= base_url().'admin/master_data/bidang/cetak_pdf'; ?>">
                                    <i class="fa fa-print"></i>&nbsp;
                                    <span class="bold">Cetak</span>
                                </a>
                                <a class="btn btn-success btn-sm" type="button" href="<?= base_url().'admin/master_data/bidang/form_tambah_bidang'; ?>">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-nowrap p-2 text-center">
                                      <a type="button" href="<?= base_url().'admin/master_data/bidang/form_ubah_bidang/'.encrypt_url($b['id_bidang']); ?>" class="text-white btn btn-xs btn-success" title="Edit Data"><i class="fa fa-pencil"></i></a>
                                      <a type="button" href="#" data-target="#hapusbid<?= encrypt_url($b['id_bidang']) ?>" data-toggle="modal" class="text-white btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
                                    </td>
                                    <td class="text-nowrap p-2"><?= lindungi($b['nm_bidang']) ?></td>
                                    <td class="text-nowrap p-2 text-center">
                                        <input type="checkbox" data-init-plugin="switchery" name="status_bidang" data-size="small" data-color="primary" value="<?= encrypt_url($b['id_bidang']) ?>" <?= $b['status_bidang']==1 ? 'title="Aktif" checked':'title="Tidak Aktif"'; ?> />
                                    </td>
                                </tr>
                                <!-- MODAL AKTIFKAN DATA -->
                                <div class="modal fade slide-up" id="hapusbid<?= encrypt_url($b['id_bidang']) ?>" tabindex="-1" role="dialog" aria-hidden="true">
                                        ...
                                        <div class="modal-content-wrapper">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    <a type="button" href="<?= base_url().'admin/master_data/aksi_bidang/hapus_bidang/'.encrypt_url($b['id_bidang']); ?>" class="btn btn-danger btn-cons pull-left inline">Yes</a>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('input[name=status_bidang]').change(function(){
                var mode= $(this).prop('checked');
                var id=$( this ).val();
                $.ajax({
                    url : "<?php echo base_url();?>admin/get_data/update_status_bidang",
                    method : "POST",
                    data : {mode:mode, id:id},
                    dataType : 'json',
                    success: function(data){
                        for(i=0; i<data.length; i++){
                            if(data[i].status_bidang==1){
                                $('input[name=status_bidang]').attr('checked', 'checked').attr('title', 'Aktif');
                            }else{
                                $('input[name=status_bidang]').removeAttr('checked', 'checked').attr('title', 'Tidak Aktif');
                            }
                        }
