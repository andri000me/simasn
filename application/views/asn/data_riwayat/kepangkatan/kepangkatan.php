                        ...
                        <div class="card-header  d-flex justify-content-between">
                            ...
                            <div class="clearfix"></div>
                                ...
                                <div class="pull-right">
                                    <div class="col-xs-12">
                                        <?php if(count($modpangkat) > 0){ ?>
                                            <a href="<?= base_url("data_riwayat/kepangkatan/tambah_kepangkatan")?>" type="button" class="btn btn-info btn-cons"><i class="fa fa-plus"></i> Tambah</a>
                                        <?php }?>
                                    </div>
                                </div>
                            ...
                            <table class="table table-striped table-hover" id="tableWithExportOptions">
                                <thead class="bg-info-lighter">
                                    <tr>
                                        ...
                                        <th class="text-black">Gol</th>
                                        <th class="text-black">TMT</th>
                                        <th class="text-black">Jenis</th>
                                        <th class="text-black">Masa Kerja</th>
                                        <th class="text-black">No.BAKN</th>
                                        <th class="text-black">Tanggal BAKN</th>
                                        <th class="text-black">No.SK</th>
                                ...
                                <tbody>
                                    <?php $no=1; foreach($modpangkat as $mp ): ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= $mp['golongan'] ?></td>
                                        <td><?= $mp['tmt_pangkat'] ? date('d-m-Y', strtotime($mp['tmt_pangkat'])) : '' ?></td>
                                        <td><?= $mp['nm_kepangkatan'] ?></td>
                                        <td><?= $mp['masa_kerja_golongan_tahun'].' Tahun '.$mp['masa_kerja_golongan_bulan'].' Bulan'; ?></td>
                                        <td><?= $mp['no_bakn'] ?></td>
                                        <td><?= $mp['tgl_bakn'] ? date('d-m-Y', strtotime($mp['tgl_bakn'])) : '' ?></td>
                                        <td><?= $mp['sk_nomor'] ?></td>
                                        <td><?= $mp['sk_tanggal'] ? date('d-m-Y', strtotime($mp['sk_tanggal'])) : '' ?></td>
                                        <td><?= $mp['pejabat_penetap'] ?></td>
                                        <td>
                                            <?php if($mp['status_aktif'] != 1): ?>
                                                <?php if($mp['validator_modpangkat']==1): ?>
                                                    <?php if($mp['status_validasi_modpangkat'] ==1):?>
                                                        <span class="bg-warning text-center text-white p-1 text-nowrap rounded"> Validasi Tahap 1 OPD </span>
                                                    <?php else:?>
                                                        <a href="#" data-toggle="modal" data-target="#valpa<?= $mp['id'] ?>"><span class="bg-danger text-center text-white p-1 text-nowrap rounded"> Gagal Validasi Tahap 1 </span></a>
                                                    <?php endif;?>
                                                <?php elseif($mp['validator_modpangkat']==2):?>
                                                    <?php if($mp['status_validasi_modpangkat'] ==1):?>
                                                        <span class="bg-warning text-center text-white p-1 text-nowrap rounded"> Validasi Tahap 2 BKPSDM </span>
                                                    <?php else:?>
                                                        <a href="#" data-toggle="modal" data-target="#valpa<?= $mp['id'] ?>"><span class="bg-danger text-center text-white p-1 text-nowrap rounded"> Gagal Validasi Tahap 2 </span></a>
                                                    <?php endif;?>
                                                <?php else: ?>
                                                    <a href="#" data-toggle="modal" data-target="#valpa<?= $mp['id'] ?>"><span class="bg-danger text-center text-white p-1 text-nowrap rounded"> Ditolak </span></a>
                                                <?php endif; ?>
                                            <?php else:?>
                                        ...
                                        <td>
                                            <a <?= $mp['status_validasi_modpangkat']==0 || $mp['status_validasi_modpangkat']==1 && $mp['status_aktif']==1 ? 'href="'.base_url("data_riwayat/kepangkatan/edit_kepangkatan/".encrypt_url($mp['id'])).'"':'href="#"  data-toggle="modal" data-target="#pkt'.$mp['id'].'"';?> type="button" class="btn-xs btn-info"><i class="fa fa-pencil"></i></a>
                                            <a href="#" data-toggle="modal" data-target="#apuspkt<?= $mp['id'] ?>" type="button" class="btn-xs btn-danger"><i class="fa fa-trash"></i></a>
                                        </td>
                                    </tr>
                                    <div class="modal fade slide-up disable-scroll" id="valpa<?= $mp['id'] ?>" tabindex="-1" role="dialog" aria-hidden="false">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content-wrapper">
                                                <div class="modal-content" style="background-color: #e2e2e2;">
                                                    <div class="modal-body text-center m-t-20">
                                                        <h6 class="no-margin p-b-10"><b>Keterangan Validasi : </b> <?= $mp['ket_validasi_modpangkat']; ?></h6>
                                    ...
                                    <div class="modal fade slide-up disable-scroll" id="pkt<?=$mp['id']?>" tabindex="-1" role="dialog" aria-hidden="false">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content-wrapper">
                                                <div class="modal-content" style="background-color: #e2e2e2;">
                                                    <div class="modal-body text-center m-t-20">
                                                        <h6 class="no-margin p-b-10">Data sementara dalam proses validasi tahap <?= $mp['validator_modpangkat'] ?>, Data tdiak bisa diedit</h6>
                                                        <button type="button" class="btn btn-xs btn-success btn-cons" data-dismiss="modal">Tutup</button>
                                    ...
                                    <div class="modal fade slide-up disable-scroll" id="apuspkt<?= $mp['id'] ?>" tabindex="-1" role="dialog" aria-hidden="false">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content-wrapper">
                                                <div class="modal-content" style="background-color: #e2e2e2;">
                                                    <div class="modal-body text-center m-t-20">
                                                        <h6 class="no-margin p-b-10">Apa anda yakin inginmenghapus data ini ? !!!</h6>
                                                        <a href="<?= base_url('data_riwayat/hapus_pangkat/'.encrypt_url($mp['id'])) ?>" type="button" class="btn btn-xs btn-success btn-cons  pull-left inline">Ya</a>
