                    ...
                    <div class="card card-transparent">
                        <div class="card-header  d-flex justify-content-between">
                            ...
                            <div class="pull-right">
                                <div class="col-xs-12">
                                    <a href="<?= base_url("data_riwayat/pendidikan/tambah_pendidikan")?>" type="button" class="btn btn-info btn-cons"><i class="fa fa-plus"></i> Tambah</a>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="table table-striped table-hover" id="tableWithExportOptions">
                                <thead class="bg-info-lighter">
                                    <tr>
                                        ...
                                        <th class="text-black">Jenis Pendidikan</th>
                                        <th class="text-black">Jenjang</th>
                                        <th class="text-black">Jurusan</th>
                                        <th class="text-black">Lembaga Pendidikan</th>
                                        <th class="text-black">Nama Kepala</th>
                                        <th class="text-black">No. Ijazah</th>
                                        <th class="text-black">Tanggal Lulus</th>
                                        <th class="text-black">Status Validasi</th>
                                        <th class="text-black">#</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php $no=1; foreach($pendidikan as $pd): ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= $pd['jenis_pendidikan']==2 ? 'Profesi' : ($pd['jenis_pendidikan']==1 ? 'Akademik' : '-'); ?></td>
                                        <td><?= !empty($pd['nm_jpendidikan']) ? $pd['nm_jpendidikan'] : '-'; ?></td>
                                        <td><?= $pd['jurusan']; ?></td>
                                        <td><?= $pd['nm_lembaga']; ?></td>
                                        <td><?= $pd['nm_kepala']; ?></td>
                                        <td><?= $pd['no_ijazah']; ?></td>
                                        <td><?= isset($pd['tgl_lulus']) ? date('d-m-Y', strtotime($pd['tgl_lulus'])) : '' ?></td>
                                        <td>
                                            <?php if($pd['status_aktif'] != 1): ?>
                                                <?php if($pd['validator_modpendidikan']==1): ?>
                                                    ...
                                                    <?php else:?>
                                                        <a href="#" data-toggle="modal" data-target="#valedu<?= $pd['id'] ?>"><span class="bg-danger text-center text-white p-1 text-nowrap rounded"> Gagal Validasi Tahap 1 </span></a>
                                                    <?php endif;?>
                                                <?php elseif($pd['validator_modpendidikan']==2):?>
                                                    <?php if($pd['status_validasi_modpendidikan'] ==1):?>
                                                        <span class="bg-warning text-center text-white p-1 text-nowrap rounded"> Validasi Tahap 2 BKPSDM </span>
                                                    <?php else:?>
                                                        <a href="#" data-toggle="modal" data-target="#valedu<?= $pd['id'] ?>"><span class="bg-danger text-center text-white p-1 text-nowrap rounded"> Gagal Validasi Tahap 2 </span></a>
                                                    <?php endif;?>
                                                <?php else: ?>
                                                    <a href="#" data-toggle="modal" data-target="#valedu<?= $pd['id'] ?>"><span class="bg-danger text-center text-white p-1 text-nowrap rounded"> Ditolak </span></a>
                                                <?php endif; ?>
                                            <?php else:?>
                                        ...
                                        <td>
                                            <a <?= $pd['status_validasi_modpendidikan']==0 || $pd['status_validasi_modpendidikan']==1 && $pd['status_aktif']==1 ? 'href="'.base_url("data_riwayat/pendidikan/ubah_pendidikan/".encrypt_url($pd['id'])).'"':'href="#"  data-toggle="modal" data-target="#edu'.$pd['id'].'"';?> type="button" class="btn-xs btn-info"><i class="fa fa-pencil"></i></a>
                                            <a href="#" data-toggle="modal" data-target="#apusedu<?= $pd['id'] ?>" type="button" class="btn-xs btn-danger"><i class="fa fa-trash"></i></a>
                                        </td>
                                    </tr>
                                    <div class="modal fade slide-up disable-scroll" id="valedu<?= $pd['id'] ?>" tabindex="-1" role="dialog" aria-hidden="false">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content-wrapper">
                                                <div class="modal-content" style="background-color: #e2e2e2;">
                                                    <div class="modal-body text-center m-t-20">
                                                        <h6 class="no-margin p-b-10"><b>Keterangan Validasi : </b> <?= $pd['ket_validasi_modpendidikan']; ?></h6>
                                    ...
                                    <div class="modal fade slide-up disable-scroll" id="edu<?=$pd['id']?>" tabindex="-1" role="dialog" aria-hidden="false">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content-wrapper">
                                                <div class="modal-content" style="background-color: #e2e2e2;">
                                                    <div class="modal-body text-center m-t-20">
                                                        <h6 class="no-margin p-b-10">Data sementara dalam proses validasi tahap <?= $pd['validator_modpendidikan'] ?>, Data tdiak bisa diedit</h6>
                                                        <button type="button" class="btn btn-xs btn-success btn-cons" data-dismiss="modal">Tutup</button>
                                    ...
                                    <div class="modal fade slide-up disable-scroll" id="apusedu<?= $pd['id'] ?>" tabindex="-1" role="dialog" aria-hidden="false">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content-wrapper">
                                                <div class="modal-content" style="background-color: #e2e2e2;">
                                                    <div class="modal-body text-center m-t-20">
                                                        <h6 class="no-margin p-b-10">Apa anda yakin inginmenghapus data ini ? !!!</h6>
                                                        <a href="<?= base_url('data_riwayat/hapus_pendidikan/'.encrypt_url($pd['id'])) ?>" type="button" class="btn btn-xs btn-success btn-cons  pull-left inline">Ya</a>
