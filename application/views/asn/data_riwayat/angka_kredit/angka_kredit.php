                    ...
                    <div class="card card-transparent">
                        <div class="card-header  d-flex justify-content-between">
                            ...
                            <div class="pull-right">
                                <div class="col-xs-12">
                                    <a href="<?= base_url("data_riwayat/angka_kredit/tambah_kredit")?>" type="button" class="btn btn-info btn-cons"><i class="fa fa-plus"></i> Tambah</a>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="table table-striped table-hover" id="tableWithExportOptions">
                                <thead class="bg-info-lighter">
                                    <tr>
                                        ...
                                        <th class="text-black">Kredit Utama</th>
                                        <th class="text-black">Kredit Penunjang</th>
                                        <th class="text-black">Total Kredit</th>
                                        <th class="text-black">Jabatan</th>
                                        <th class="text-black">Status Validasi</th>
                                        <th class="text-black">#</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php $no=1; foreach($kredit as $kr): ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= $kr['no_sk']; ?></td>
                                        <td><?= isset($kr['tgl_sk']) ? date('d-m-Y', strtotime($kr['tgl_sk'])) : '' ?></td>
                                        <td><?= $kr['kredit_utama']; ?></td>
                                        <td><?= $kr['kredit_penunjang']; ?></td>
                                        <td><?php $kredit = array($kr['kredit_utama'], $kr['kredit_penunjang']); echo array_sum($kredit) ?></td>
                                        <td><?= $kr['nm_jabatan']; ?></td>
                                        <td>
                                            <?php if($kr['status_aktif'] != 1): ?>
                                                <?php if($kr['validator_modkredit']==1): ?>
                                                    ...
                                                    <?php else:?>
                                                        <a href="#" data-toggle="modal" data-target="#valkre<?= $kr['id'] ?>"><span class="bg-danger text-center text-white p-1 text-nowrap rounded"> Gagal Validasi Tahap 1 </span></a>
                                                    <?php endif;?>
                                                <?php elseif($kr['validator_modkredit']==2):?>
                                                    <?php if($kr['status_validasi_modkredit'] ==1):?>
                                                        <span class="bg-warning text-center text-white p-1 text-nowrap rounded"> Validasi Tahap 2 BKPSDM </span>
                                                    <?php else:?>
                                                        <a href="#" data-toggle="modal" data-target="#valkre<?= $kr['id'] ?>"><span class="bg-danger text-center text-white p-1 text-nowrap rounded"> Gagal Validasi Tahap 2 </span></a>
                                                    <?php endif;?>
                                                <?php else: ?>
                                                    <a href="#" data-toggle="modal" data-target="#valkre<?= $kr['id'] ?>"><span class="bg-danger text-center text-white p-1 text-nowrap rounded"> Ditolak </span></a>
                                                <?php endif; ?>
                                            <?php else:?>
                                        ...
                                        <td>
                                            <a <?= $kr['status_validasi_modkredit']==0 || $kr['status_validasi_modkredit']==1 && $kr['status_aktif']==1 ? 'href="'.base_url("data_riwayat/angka_kredit/ubah_kredit/".encrypt_url($kr['id'])).'"':'href="#"  data-toggle="modal" data-target="#msgkre'.$kr['id'].'"';?> type="button" class="btn-xs btn-info"><i class="fa fa-pencil"></i></a>
                                            <a href="#" data-toggle="modal" data-target="#apuskre<?= $kr['id'] ?>" type="button" class="btn-xs btn-danger"><i class="fa fa-trash"></i></a>
                                        </td>
                                        
                                    </tr>
                                    <div class="modal fade slide-up disable-scroll" id="valkre<?= $kr['id'] ?>" tabindex="-1" role="dialog" aria-hidden="false">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content-wrapper">
                                                <div class="modal-content" style="background-color: #e2e2e2;">
                                                    <div class="modal-body text-center m-t-20">
                                                        <h6 class="no-margin p-b-10"><b>Keterangan Validasi : </b> <?= $kr['ket_validasi_modkredit']; ?></h6>
                                    ...
                                    <div class="modal fade slide-up disable-scroll" id="msgkre<?=$kr['id']?>" tabindex="-1" role="dialog" aria-hidden="false">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content-wrapper">
                                                <div class="modal-content" style="background-color: #e2e2e2;">
                                                    <div class="modal-body text-center m-t-20">
                                                        <h6 class="no-margin p-b-10">Data sementara dalam proses validasi tahap <?= $kr['validator_modkredit'] ?>, Data tdiak bisa diedit</h6>
                                                        <button type="button" class="btn btn-xs btn-success btn-cons" data-dismiss="modal">Tutup</button>
                                    ...
                                    <div class="modal fade slide-up disable-scroll" id="apuskre<?= $kr['id'] ?>" tabindex="-1" role="dialog" aria-hidden="false">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content-wrapper">
                                                <div class="modal-content" style="background-color: #e2e2e2;">
                                                    <div class="modal-body text-center m-t-20">
                                                        <h6 class="no-margin p-b-10">Apa anda yakin inginmenghapus data ini ? !!!</h6>
                                                        <a href="<?= base_url('data_riwayat/hapus_kredit/'.encrypt_url($kr['id'])) ?>" type="button" class="btn btn-xs btn-success btn-cons  pull-left inline">Ya</a>
